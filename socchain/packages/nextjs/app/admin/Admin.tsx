...
import { 
  Shield, Settings, Users, Vote, Award, AlertCircle, CheckCircle, 
  Loader2, Hash, Play, XCircle
} from 'lucide-react';
...
const NOMINATION_ABI = [
  {"inputs": [{"internalType": "uint96","name": "_nominationStart","type": "uint96"},{"internalType": "uint96","name": "_nominationEnd","type": "uint96"},{"internalType": "uint256","name": "_topN","type": "uint256"},{"internalType": "uint256","name": "_minimumNominations","type": "uint256"},{"internalType": "bool","name": "_autoFinalizationEnabled","type": "bool"}],"name": "createElection","outputs": [{"internalType": "uint256","name": "","type": "uint256"}],"stateMutability": "nonpayable","type": "function"},
  {"inputs": [{"internalType": "uint256","name": "_electionId","type": "uint256"}],"name": "finalizeNominations","outputs": [],"stateMutability": "nonpayable","type": "function"},
  {"inputs": [],"name": "electionCounter","outputs": [{"internalType": "uint256","name": "","type": "uint256"}],"stateMutability": "view","type": "function"}
];

const VETTING_ABI = [
  {"inputs": [{"internalType": "uint256","name": "_electionId","type": "uint256"},{"internalType": "uint256","name": "_jurySize","type": "uint256"},{"internalType": "uint96","name": "_commitDuration","type": "uint96"},{"internalType": "uint96","name": "_revealDuration","type": "uint96"},{"internalType": "uint256","name": "_stakeAmount","type": "uint256"},{"internalType": "bool","name": "_autoTransitionEnabled","type": "bool"}],"name": "createVettingSession","outputs": [{"internalType": "uint256","name": "","type": "uint256"}],"stateMutability": "nonpayable","type": "function"},
  {"inputs": [{"internalType": "uint256","name": "_sessionId","type": "uint256"}],"name": "requestJurySelection","outputs": [{"internalType": "uint256","name": "","type": "uint256"}],"stateMutability": "nonpayable","type": "function"},
  {"inputs": [{"internalType": "uint256","name": "_sessionId","type": "uint256"}],"name": "finalizeVetting","outputs": [],"stateMutability": "nonpayable","type": "function"},
  {"inputs": [],"name": "sessionCounter","outputs": [{"internalType": "uint256","name": "","type": "uint256"}],"stateMutability": "view","type": "function"},
  {
...
const LOTTERY_ABI = [
  {"inputs": [{"internalType": "uint256","name": "_vettingSessionId","type": "uint256"},{"internalType": "uint96","name": "_votingStart","type": "uint96"},{"internalType": "uint96","name": "_votingEnd","type": "uint96"},{"internalType": "bool","name": "_autoDrawEnabled","type": "bool"}],"name": "createElection","outputs": [{"internalType": "uint256","name": "","type": "uint256"}],"stateMutability": "nonpayable","type": "function"},
  {"inputs": [{"internalType": "uint256","name": "_electionId","type": "uint256"}],"name": "requestWinnerDraw","outputs": [{"internalType": "uint256","name": "","type": "uint256"}],"stateMutability": "nonpayable","type": "function"},
  {"inputs": [],"name": "s_electionCounter","outputs": [{"internalType": "uint256","name": "","type": "uint256"}],"stateMutability": "view","type": "function"}
];

const IDENTITY_ABI = [
  {"inputs": [{"internalType": "address","name": "_member","type": "address"},{"internalType": "bool","name": "_status","type": "bool"}],"name": "setWhitelist","outputs": [],"stateMutability": "nonpayable","type": "function"},
  {"inputs": [],"name": "totalMembers","outputs": [{"internalType": "uint256","name": "","type": "uint256"}],"stateMutability": "view","type": "function"},
  {"inputs": [{"internalType": "address","name": "","type": "address"}],"name": "identities","outputs": [{"internalType": "address","name": "member","type": "address"},{"internalType": "uint96","name": "registeredAt","type": "uint96"},{"internalType": "uint96","name": "lastActivityAt","type": "uint96"},{"internalType": "uint64","name": "passportScore","type": "uint64"},{"internalType": "uint96","name": "lastScoreUpdate","type": "uint96"},{"internalType": "uint256","name": "participationCount","type": "uint256"},{"internalType": "uint256","name": "stakeAmount","type": "uint256"},{"internalType": "bool","name": "isWhitelisted","type": "bool"},{"internalType": "bool","name": "isBanned","type": "bool"}],"stateMutability": "view","type": "function"},
  {"inputs": [],"name": "getAllMembers","outputs": [{"internalType": "address[]","name": "","type": "address[]"}],"stateMutability": "view","type": "function"}
];

const CANDIDACY_ABI = [
  {"inputs": [{"internalType": "address","name": "_vettingContract","type": "address"}],"name": "grantVettingRole","outputs": [],"stateMutability": "nonpayable","type": "function"},
  {"inputs": [{"internalType": "uint256","name": "_tokenId","type": "uint256"}],"name": "disqualifyCandidate","outputs": [],"stateMutability": "nonpayable","type": "function"}
];

// Member Row Component with Real-time Whitelist Status
const MemberRow = ({ address: memberAddress, index, selectedJurors, onToggleWhitelist, isProcessing }: any) => {
...
export default function AdminDashboard() {
  ...
  const isAlreadyMember = memberCheck && memberCheck[0] !== '0x0000000000000000000000000000000000000000';
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={step.num} className="flex items-start gap-3 bg-white/10 rounded-lg p-3">
              <div className="w-8 h-8 bg-white text-indigo-600 rounded-full flex items-center justify-center font-bold flex-shrink-0">{step.num}</div>
              <div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Top N Candidates</label>
            <input
              type="number"
              value={topN}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Minimum Nominations</label>
            <input
              type="number"
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        ...
        <div className="space-y-3">
          ...
          >
            {isFinalizingNom ? <Loader2 className="w-3 h-3 animate-spin" /> : <CheckCircle className="w-3 h-3" />}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Jury Size (odd number, min 5)</label>
            <select
              value={jurySize}
            ...
            >
              {[5, 7, 9, 11, 13, 15].map(n => (
                <option key={n} value={n}>{n} Jurors</option>
              ))}
            </select>
          </div>
          
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Commit Duration (seconds)</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Reveal Duration (seconds)</label>
            <input
              type="number"
        ...
        >
          {isCreatingVetting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        ...
        <div className="bg-red-50 border-2 border-red-400 rounded-lg p-4 mb-4">
          ...
          <ol className="text-sm text-red-800 list-decimal list-inside space-y-1">
            ...
            <li><strong>IMMEDIATELY request jury selection</strong> - randomly picks jurors via VRF</li>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Request Jury Selection (VRF)</label>
            <div className="flex gap-2">
              ...
              >
                {isRequestingJury ? <Loader2 className="w-3 h-3 animate-spin" /> : <Hash className="w-3 h-3" />}
                Request
              </button>
            </div>
            {jurySuccess && (
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Finalize Vetting Session</label>
            <div className="flex gap-2">
              <input
                type="number"
                placeholder="Session ID"
              ...
              >
                {isFinalizingVetting ? <Loader2 className="w-3 h-3 animate-spin" /> : <CheckCircle className="w-3 h-3" />}
                Finalize
              </button>
            </div>
  ...
  const LotteryTab = () => (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Vetting Session ID</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Voting Start (Unix)</label>
            <input
              type="number"
              value={votingStart}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Voting End (Unix)</label>
            <input
              type="number"
              value={votingEnd}
        ...
        >
          {isCreatingLottery ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
          Create Lottery Election
        </button>
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        ...
        <div className="space-y-3">
          ...
          >
            {isRequestingDraw ? <Loader2 className="w-3 h-3 animate-spin" /> : <Hash className="w-3 h-3" />}
      ...
      <div className="space-y-6">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl p-6 text-white">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            >
              {vettingCounter && Array.from({ length: Number(vettingCounter) }, (_, i) => i + 1).map(id => (
                <option key={id} value={id} className="text-slate-800">Session #{id}</option>
              ))}
            </select>
          </div>
          ...
          {vettingSession && (
            <div className="space-y-3">
              <div className="bg-white/10 rounded-lg p-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-2xl font-bold">{PHASE_NAMES[vettingSession[7]]}</div>
                  </div>
                  <div>
              ...
              {selectedJurors.length > 0 ? (
                <div className="bg-white/10 rounded-lg p-4">
                  <h4 className="font-semibold mb-3 text-lg">✓ Selected Jurors for Session #{selectedSessionId}</h4>
          ...
          {allMembers && allMembers.length > 0 && (
            <div className="mt-4 p-4 bg-indigo-50 rounded-lg">
              <p className="text-sm text-indigo-800">
                <strong>✓ {allMembers.length}</strong> member{allMembers.length !== 1 ? 's' : ''} registered. 
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          ...
          <div className="space-y-3">
            ...
            {whitelistAddress && whitelistAddress.length === 42 && whitelistAddress.startsWith('0x') && (
              <>
            ...
            <button
              ...
              disabled={isSettingStatus || !whitelistAddress || (isAlreadyMember && isAlreadyWhitelisted)}
            ...
            >
              {isSettingStatus ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />}
              {isAlreadyMember && isAlreadyWhitelisted ? 'Already Whitelisted' : 'Whitelist Member'}
            </button>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          ...
          <div className="space-y-3">
            ...
            >
              {isDisqualifying ? <Loader2 className="w-4 h-4 animate-spin" /> : <XCircle className="w-4 h-4" />}
              Disqualify Candidate
            </button>
          </div>
          {disqualifySuccess && (
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          ...
          <div className="space-y-3">
            <button
              ...
              className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-400 text-white font-semibold py-3 rounded-lg flex items-center justify-center gap-2"
            >
              {isGranting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Shield className="w-4 h-4" />}
      ...
      <div className="container mx-auto px-4">
        <div className="mb-8">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="px-2 py-1 bg-green-100 text-green-700 rounded-full text-xs font-medium">Admin</span>
          </div>
        </div>
