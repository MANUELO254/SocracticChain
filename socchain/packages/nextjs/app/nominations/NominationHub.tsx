      ...
      <div className="bg-indigo-50 border-2 border-indigo-400 rounded-xl p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              {electionCounter &&
                Array.from({ length: Number(electionCounter) }, (_, i) => i + 1).map((id) => (
        ...
        {election && (
          <div className="mt-3 text-sm text-indigo-800 space-y-1">
            ...
            <div>
              Top {election.topN} candidates will advance • Min {election.minimumNominations} nominations required
            </div>
          </div>
        )}
      </div>
        ...
        <div className="flex items-start gap-6 mb-6">
          ...
          <div className="flex-1">
            <h3 className="text-2xl font-bold text-slate-800 mb-2">
              Election #{electionId} - Candidacy Declaration
            </h3>
            <p className="text-slate-600 mb-2">
              Mint your Candidacy NFT to declare your intent to run
            </p>
        ...
        {!address ? (
          ...
          <div className="p-4 bg-amber-50 rounded-xl text-center">
            ...
            <p className="text-amber-800 font-medium">
              {!election
                ...
                : election.isFinalized
                ? "This election has been finalized"
                : now < election.nominationStart
                ? `Election starts ${new Date(
                    election.nominationStart * 1000
                  ).toLocaleString()}`
          ...
          <div className="p-4 bg-blue-50 rounded-xl text-center">
            ...
            <p className="text-blue-700 font-medium mb-3">
              You have active candidacy #{activeId?.toString()}
            </p>
            <button
          ...
          <div className="p-4 bg-green-50 rounded-xl text-center">
            ...
            <p className="text-green-700 font-medium mb-3">
              You are eligible to declare candidacy!
            </p>
            <button
              onClick={() => setActiveTab("declare")}
          ...
          <div className="p-4 bg-red-50 rounded-xl text-center">
            ...
            <p className="text-red-700">
              Cooldown active. Eligible in{" "}
              {Math.ceil((Number(cooldown || 0n) - cooldownElapsed) / 60)} minutes
            </p>
          </div>
        )}
      </div>
        ...
        <Link href={`/nominationsvote?election=${electionId}`}>
          <div className="p-4 border-2 border-purple-200 rounded-lg hover:border-purple-500 cursor-pointer">
            ...
            <p className="text-sm text-slate-600">
              Vote to nominate candidates for vetting
            </p>
          </div>
        </Link>
      </div>
            ...
            <div className={`text-sm ${isElectionActive ? "text-green-800" : "text-amber-800"}`}>
              {isElectionActive ? (
                <>
                  ...
                  {new Date(election.nominationEnd * 1000).toLocaleString()}
                </>
              ) : (
                <>
                  ...
                  {election.isFinalized
                    ? " This election has been finalized."
                    : now < election.nominationStart
                    ? ` Starts: ${new Date(election.nominationStart * 1000).toLocaleString()}`
                    : " Nomination period has ended."}
                </>
              )}
            </div>
      ...
      {!address ? (
          ...
          {profile && (
            <div className="space-y-4">
              <div>
                ...
                <span className="font-medium">#{profile.electionId.toString()}</span>
              </div>
              <div>
                <span className="text-sm text-slate-600">Summary:</span>{" "}
                <p className="font-medium">{profile.platformSummary}</p>
              </div>
              <div>
                <span className="text-sm text-slate-600">IPFS:</span>{" "}
                <a
                  href={`https://ipfs.io/ipfs/${profile.platformIPFS}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-blue-600 hover:underline"
                >
                  {profile.platformIPFS.slice(0, 20)}...
                </a>
              </div>
              <div>
              ...
              <div className="text-sm text-slate-500">
                Minted:{" "}
                {new Date(Number(profile.mintedAt) * 1000).toLocaleDateString()}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="bg-amber-50 border-l-4 border-amber-400 p-4 rounded">
          ...
          <span className="text-amber-800">
            Cannot declare candidacy - election is not in nomination phase
          </span>
        </div>
      ) : (
        <div className="bg-white rounded-xl shadow-md p-6 space-y-4">
          ...
          <div>
            ...
            <div className="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center">
              ...
              <label htmlFor="file-upload" className="cursor-pointer">
                ...
                <p className="text-sm text-slate-600">
                  Click to upload your platform document
                </p>
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="governance, sustainability, innovation"
              value={tagsInput}
      ...
      <div className="container mx-auto px-4">
        <div className="mb-8">
          ...
          <p className="text-slate-600">
            Mint your Candidacy NFT to enter the election process
          </p>
        </div>
