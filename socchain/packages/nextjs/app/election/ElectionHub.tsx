...
import { Award, BarChart3, Clock, Crown, Loader2, TrendingUp, Users } from "lucide-react";
...
const LOTTERY_ABI = [
  {
    ...
    name: "getElection",
    outputs: [
      { internalType: "uint256", name: "vettingSessionId", type: "uint256" },
      { internalType: "uint256[]", name: "candidateIds", type: "uint256[]" },
      { internalType: "uint96", name: "votingStart", type: "uint96" },
    ...
    outputs: [
      {
        components: [
          ...
          { internalType: "uint256", name: "probabilityBasisPoints", type: "uint256" },
        ],
...
export default function ElectionHub() {
  ...
  const rawElection = electionData as
    | [bigint, bigint[], bigint, bigint, boolean, bigint, bigint, bigint]
    | undefined;

  const election: Election | null = rawElection
    ? {
        vettingSessionId: Number(rawElection[0]),
        candidateIds: rawElection[1].map((id) => Number(id)),
        votingStart: Number(rawElection[2]),
        votingEnd: Number(rawElection[3]),
        isFinalized: rawElection[4],
        winner: Number(rawElection[5]),
        totalVotes: Number(rawElection[6]),
        totalWeight: Number(rawElection[7]),
      }
    : null;
  ...
  const distributions = voteDistribution as VoteDistribution[] | undefined;
              ...
              <tbody>
                {distributions && distributions.length > 0 ? (
                  distributions.map((dist, idx) => {
                    ...
                    return (
                      <tr key={idx} className={`border-b border-slate-100 ${isWinner ? "bg-yellow-50" : ""}`}>
                        ...
                        <td className="py-3 px-4 text-slate-600 max-w-xs truncate">{dist.platformSummary}</td>
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            <TrendingUp className="w-4 h-4 text-indigo-500" />
                            <span className="font-semibold text-slate-800">{dist.voteWeight.toString()}</span>
                          </div>
                        </td>
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            <div className="flex-1 bg-slate-200 rounded-full h-2 max-w-[100px]">
                              <div className="bg-indigo-600 h-2 rounded-full" style={{ width: `${probability}%` }} />
                            </div>
                            <span className="font-semibold text-indigo-600 text-sm">{probability}%</span>
                          </div>
                        </td>
                        <td className="py-3 px-4">
                          {isWinner ? (
