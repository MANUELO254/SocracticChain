...
export default function NominationVotingPage() {
  ...
  const [electionId, setElectionId] = useState(
    electionParam ? parseInt(electionParam) : 1
  );
  ...
  const { data: candidateProfiles } = useReadContract({
    ...
    args:
      activeCandidateIds && activeCandidateIds.length > 0
        ? [activeCandidateIds]
        : undefined,
  });
      ...
      <div className="container mx-auto px-4">
        <div className="mb-8">
          ...
          <p className="text-slate-600">
            Vote to nominate candidates for the vetting phase
          </p>
        </div>
        ...
        <div className="bg-indigo-50 border-2 border-indigo-400 rounded-xl p-4 mb-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              >
                {electionCounter &&
                  Array.from({ length: Number(electionCounter) }, (_, i) => i + 1).map(
                    (id) => (
            ...
            <div className="flex items-start gap-6 mb-6">
              ...
              <div className="flex-1">
                <h3 className="text-2xl font-bold text-slate-800 mb-2">
                  Election #{electionId} Nomination Phase
                </h3>
                <p className="text-slate-600 mb-2">
                  Select up to 10 candidates to nominate for vetting
                </p>
        ...
        {!address ? (
          <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded mb-8">
            ...
            <span className="text-yellow-800">
              Connect your wallet to nominate candidates
            </span>
          </div>
        ) : !election ? (
          ...
          <div className="bg-red-50 border-l-4 border-red-400 p-4 rounded mb-8">
            ...
            <span className="text-red-800">
              {election.isFinalized
                ? "Nomination phase has ended and been finalized"
                : now < election.nominationStart
                ? `Nomination phase starts ${new Date(
          ...
          <div className="bg-green-50 border-l-4 border-green-400 p-4 rounded mb-8">
            ...
            <span className="text-green-800">
              You nominated candidates:{" "}
              {voterNominations?.map((id) => `#${id}`).join(", ")}
            </span>
          </div>
        ) : null}
            ...
            >
              {isVoting ? (
                ...
                <>
                  ...
                  Nominate {selectedCandidates.length}{" "}
                  ...
                  leaderboard.map((entry: any, idx: number) => (
                    <tr key={idx} className="border-b border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="py-3 px-4">
                        <div className="font-medium text-slate-800">
                          Candidate #{entry.candidateId.toString()}
                        </div>
                        <div className="text-xs text-slate-500">
                          {entry.candidateAddress.slice(0, 6)}...
                          {entry.candidateAddress.slice(-4)}
                        </div>
                      </td>
                      <td className="py-3 px-4 text-slate-600 max-w-xs truncate">
                        {entry.platformSummary}
                      </td>
