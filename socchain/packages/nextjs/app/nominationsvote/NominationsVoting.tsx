...
import { AlertCircle, Award, CheckCircle, Clock, Loader2, Users, Vote } from "lucide-react";
...
const CANDIDACY_ABI = [
  ...
  },
  {
    ...
    name: "getBatchCandidateProfiles",
    outputs: [
      {
        components: [
...
export default function NominationVotingPage() {
  ...
  const { data: candidateProfiles } = useReadContract({
    ...
    args:
      activeCandidateIds && (activeCandidateIds as bigint[]).length > 0
        ? [activeCandidateIds as bigint[]]
        : undefined,
  });
  ...
  const profiles = candidateProfiles as CandidateProfile[] | undefined;
  const nominations = voterNominations as bigint[] | undefined;
  const leaderboardEntries = leaderboard as LeaderboardEntry[] | undefined;
      ...
      <div className="container mx-auto px-4">
          ...
          <div className="flex items-center justify-between">
            ...
            {election && (
              <div className="flex items-center gap-2">
                {isActive ? (
                  <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm font-medium">● Voting Active</span>
        ...
        {election && (
          <div className="bg-white rounded-2xl shadow-lg p-8 border-t-4 border-indigo-500 mb-8">
            ...
            <div className="grid grid-cols-3 gap-6 text-center">
              <div>
                <div className="text-3xl font-bold text-indigo-600">{(activeCandidateIds as bigint[] | undefined)?.length || 0}</div>
        ...
        {!address ? (
          ...
          <div className="bg-green-50 border-l-4 border-green-400 p-4 rounded mb-8">
            ...
            <span className="text-green-800">
              You nominated candidates: {nominations?.map((id) => `#${id.toString()}`).join(", ")}
            </span>
          </div>
        ) : null}
        ...
        {isActive && !hasVoted && address && (
          <div className="bg-white rounded-xl shadow-md p-6 mb-8">
            <h3 className="font-semibold text-slate-800 mb-4">Cast Your Nominations ({selectedCandidates.length}/10 selected)</h3>
            ...
            <div className="space-y-3 mb-6 max-h-96 overflow-y-auto">
              {profiles && profiles.length > 0 ? (
                ...
                <div className="text-center py-8 text-slate-500">No active candidates for this election yet</div>
              )}
            </div>
            ...
            {voteSuccess && <div className="mt-4 p-3 bg-green-100 rounded text-green-700 text-sm">✓ Nominations cast successfully!</div>}
          </div>
        )}
                ...
                {leaderboardEntries && leaderboardEntries.length > 0 ? (
                  leaderboardEntries.map((entry, idx) => (
                    <tr key={idx} className="border-b border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="py-3 px-4 text-lg font-bold text-indigo-600">{entry.nominationCount.toString()}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
